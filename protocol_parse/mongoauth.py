...
class MongoDBAuth(object):
    ...
    __DOCUMENT_DATA_TYPE = {
        ...
        '05': 'binary',  # length
        '08': 'boolean',  # one byte
        '09': 'datetime',  # eight bytes
        '01': 'double',  # eight bytes
        '04': 'array',
        '12': 'int64',  # eight bytes
        '11': 'timestamp',
    ...
    def parse_data(self, sep='\x00'):
        ...
        if auth_detail and auth_result:
            ...
            # Simultaneous transfer of account and password
    ...
    def __parse_document(self, documents):
        ...
        # documentlength 4
        # elements
        # Elements composition: type 1 + elementName length is not fixed, ending with 00 + [length] + value length is determined by type
    ...
    def __split_mongo_data(self, data):
        ...
        while offset < len_of_chrs:
            # https://docs.mongodb.com/manual/reference/mongodb-wire-protocol/
            # Protocol format: the first 4 bytes indicate the length of the message
            # Byte order adopts: little endian byte order
